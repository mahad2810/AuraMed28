...
def get_coordinates(address):
    ...
    api_key = "AIzaSyA2Zj2WO5oVgLhQtW0x9JvV_Ns9pUxE6Lc"
    ...
    if response.status_code == 200:
        data = response.json()
        if data["status"] == "OK":  # Check if response is valid
            location = data["results"][0]["geometry"]["location"]
            print(f"Coordinates found: {location}")  # Debug statement
            return location["lat"], location["lng"]
        else:
            print(f"Error from Google API: {data['status']}")  # Debug statement
    else:
        print(f"Google API HTTP error: {response.status_code}")  # Debug statement
...
@home_bp.route('/api/add_relative', methods=['POST'])
def add_relative():
    """Add a relative to the user's profile"""
    try:
        data = request.json
        ...
        if not all([user_email, relative_name, relative_email, relative_phone, relation]):
            return jsonify({'message': 'All fields are required'}), 400
            
        # Create relative entry
        relative_entry = {
        ...
        # Insert into relatives collection
        relatives_collection = current_app.mongo.db.relatives
        result = relatives_collection.insert_one(relative_entry)
        
        if result.inserted_id:
            return jsonify({
                'message': 'Relative added successfully',
                'relative_id': str(result.inserted_id)
            })
        else:
            return jsonify({'message': 'Failed to add relative'}), 500
            
    except Exception as e:
        print(f"Error adding relative: {str(e)}")
