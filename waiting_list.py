...
@waiting_list_bp.route("/get_video_calls", methods=["GET"])
def get_video_calls():
    try:
        # Get the logged-in hospital email or user email
        hospital_email = session.get('hospital_email')
        user_email = session.get('user_email')
        
        # Determine if this is a hospital or patient view
        if hospital_email:
            # Hospital view - get all requests for this hospital
            requests = list(video_call_collection.find({"doctor.hospital": hospital_email, "status": "request"}))
            ongoing = list(video_call_collection.find({"doctor.hospital": hospital_email, "status": "ongoing"}))
            completed = list(video_call_collection.find({"doctor.hospital": hospital_email, "status": "completed"}))
        elif user_email:
            # Patient view - get all requests from this patient
            requests = list(video_call_collection.find({"patient.email": user_email, "status": "request"}))
            ongoing = list(video_call_collection.find({"patient.email": user_email, "status": "ongoing"}))
            completed = list(video_call_collection.find({"patient.email": user_email, "status": "completed"}))
        else:
            return jsonify({"success": False, "message": "Not logged in!"}), 401
        
        # Convert ObjectId to string for JSON serialization
        for items in [requests, ongoing, completed]:
            for item in items:
                if "_id" in item:
        ...
        print(f"Error fetching video calls: {str(e)}")
